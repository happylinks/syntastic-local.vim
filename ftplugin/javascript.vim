...
" return full path of local flow executable
"  or an empty string if no executable found
fun! s:GetFlowExec (node_modules)
  let flow_guess = a:node_modules is '' ? '' : a:node_modules . '.bin/flow'
  return exepath(flow_guess)
endfun
...
fun! s:main ()
  ...
  let flow_exec = s:GetFlowExec(node_modules)
  call s:LetEslintExec(eslint_exec)
  call s:LetFlowExec(flow_exec)
endfun
